 ...
 * @param : $champTabble     champs de la table
 * @param : $numSys      num du systeme
 * 
 * 
 * 
 * 
...
function afficheTablePanneau($idcom, $nomTable, $classCss,$champTabble,$numSys,$ID,$lim) {
	$requete="SELECT `Horodatage`,`Inclinaison_Panneau`,`Orientation_Panneau`,`I_panneau`  FROM " . $nomTable." WHERE ".$champTabble."=".$numSys." ORDER BY ".$ID. " DESC LIMIT " .$lim."";
	afficheRequete($idcom, $requete, $classCss);
}
/**
 * Affiche sous forme d'un tableau le résultat d'une requête passée en paramètre
...
function afficheEnTeteColonnes($titres) {
	...
	foreach($titres as $nomcol) {
		echo " <th style='padding: 5px'> ". htmlentities($nomcol) ." </th> ";
	}
	echo "</tr> </thead>";	 
 }
 ///////////////////////////////////////////////////////////////////////////////////////////////////
